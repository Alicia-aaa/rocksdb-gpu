...
Status TransactionBaseImpl::ValueFilter(const ReadOptions& read_options,
                                        ...
                                        std::vector<PinnableSlice> &pinnable_values, char **data_buf, uint64_t *num_entries,
                                        int join_idx, double *pushdown_evaluate, double *data_transfer) {
  return write_batch_.ValueFilterFromBatchAndDB(db_, read_options,
                                                column_family, key, keys,
                                                pinnable_values, data_buf, num_entries, join_idx, pushdown_evaluate, data_transfer);
}
